
import Body from "./Body";
import Triangle from "../Shapes/Triangle";
export class Pyramid extends Body {


    comp: Triangle



    constructor(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, m: number) {
        super()

        this.comp = new Triangle(x1, y1, x2, y2, x3, y3);

        this.pos = this.comp.pos

        this.m = m;

        this.m === 0 ? this.inv_m = 0 : this.inv_m = 1 / this.m

        // let sideLength = this.comp.vertex[1].subtr(this.comp.vertex[0]).mag()
        // let height = Math.sqrt(sideLength ** 2 - sideLength / 2 ** 2)
        // this.inertia = this.m * ((sideLength ** 2 + height ** 2) * sideLength * height * 1 / 36);

        this.inertia = 2
        

        this.m === 0 ? this.inv_inertia = 0 : this.inv_inertia = 1 / this.inertia

    }


    setPosition(x: number, y: number, a = this.angle) {
        this.pos.set(x, y);
        this.angle = a;
        this.comp.pos = this.pos;
        this.comp.getVertices(this.angle + this.angVel);
        this.angle += this.angVel;
    }

    reposition() {
        super.reposition();
        this.setPosition(this.pos.add(this.vel).x, this.pos.add(this.vel).y);
    }
    keyControl() {
        if (this.up) {
            this.acc = this.comp.dir.mult(-this.keyForce);;
        }
        if (this.down) {
            this.acc = this.comp.dir.mult(this.keyForce);;
        }
        if (this.left) {
            this.angVel = -this.angKeyForce;
        }
        if (this.right) {
            this.angVel = this.angKeyForce;
        }
        if (!this.up && !this.down) {
            this.acc.set(0, 0);
        }
    }
}

